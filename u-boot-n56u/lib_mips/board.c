...
//int input_txDelay = 1, input_rxDelay = 2;
...
static int skip_atoi2(const char *s)
{
        int i=0;
...
/*
	char str_txDelay[8];
	char str_rxDelay[8];
	memset(str_txDelay, 0x0, 8);
	memset(str_rxDelay, 0x0, 8);
        printf("\tInput txDelay ");
        input_value(str_txDelay);
	input_txDelay = skip_atoi2(str_txDelay);
	printf("\tInput rxDelay ");
	input_value(str_rxDelay);
	input_rxDelay = skip_atoi2(str_rxDelay);
	printf("txDelay: %d, rxDelay: %d\n", input_txDelay, input_rxDelay);
	if (input_txDelay < 0)
		input_txDelay = 0;
	if (input_rxDelay < 0)
		input_rxDelay = 0;
...
#if HW_ANNEX_A == 1
	debug("\n This is Annex A boot loader \n");
#elif HW_ANNEX_B == 1
	debug("\n This is Annex B boot loader \n");
#elif
	#error "no HW defined";
#endif
...
#if 0
// test code to read DDR control register
	{
		int reg;
		reg = RALINK_REG(RT2880_SYSCFG1_REG);
		printf("SYSCFG1_REG : %x\n", reg);
	}
#endif	
	
#if 0
// test code to check cpu speed
	{
		int cpu_speed = mips_cpu_feq/1000/1000;
		printf("%d",cpu_speed);
		if (cpu_speed == 500)
		{
			printf("... ok\n",cpu_speed);	
...
#if DSL_N55U == 1
	// we use i2c sd as wifi on/off switch
	// we use spi clk as reset button
	set_i2c_spi_to_gpio_mode_and_dir();	
#endif		
...
//
// this code force to write boot version to 1.006
// this also check MAC address and write to a valid one
// those patch are not good
// if flash corrupt, the flaw product should return and analyze
// only write to valid mac address could not resolve any problem and do not know what happened
//
...
/*
		case '6':
			printf("   \n%d: Factory Block Init. \n", SEL_FACTORY_INIT);
			printf(" Warning!! Erase factory block then burn new one. Are you sure?(Y/N)\n");
			confirm = getc();
			if (confirm != 'y' && confirm != 'Y') {
				printf(" Operation terminated\n");
				break;
			}
			tftp_config(SEL_FACTORY_INIT, argv);
			argc= 3;
			setenv("autostart", "no");
			do_tftpb(cmdtp, 0, argc, argv);

			if (NetBootFileXferSize > CFG_FACTORY_SIZE) {
				printf("Abort: bootloader size too big!\n");
			}

			{
				//protect off uboot
				flash_sect_protect(0, CFG_FLASH_BASE+CFG_BOOTLOADER_SIZE + CFG_CONFIG_SIZE,
					CFG_FLASH_BASE+CFG_BOOTLOADER_SIZE + CFG_CONFIG_SIZE +CFG_FACTORY_SIZE-1);
				
				//erase factory block
				printf("\n Erase U-Boot block !!\n");
				printf("From 0x%X To 0x%X\n", CFG_FLASH_BASE+CFG_BOOTLOADER_SIZE + CFG_CONFIG_SIZE, 
					CFG_FLASH_BASE+CFG_BOOTLOADER_SIZE+ CFG_CONFIG_SIZE+CFG_FACTORY_SIZE-1);
				flash_sect_erase(CFG_FLASH_BASE+CFG_BOOTLOADER_SIZE + CFG_CONFIG_SIZE,
					CFG_FLASH_BASE+CFG_BOOTLOADER_SIZE+ CFG_CONFIG_SIZE+CFG_FACTORY_SIZE-1);
				...
				//protect on uboot
				flash_sect_protect(1, CFG_FLASH_BASE+CFG_BOOTLOADER_SIZE + CFG_CONFIG_SIZE,
					CFG_FLASH_BASE+CFG_BOOTLOADER_SIZE + CFG_CONFIG_SIZE +CFG_FACTORY_SIZE-1);
			}
