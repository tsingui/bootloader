...
int check_image_header(unsigned char* ImageBuf)
{
	int i;
	int ImageErr = 0;
	int sleep_ms = 30;
	unsigned char buf[64];
	memcpy(buf,ImageBuf,sizeof(buf));
	...
	{
		printf("Image header broken\n");
		ImageErr = 1;
	}
	else
	{
		printf("Image header OK\n");
	}
	
#if HW_ANNEX_A == 1
	if (strncmp(buf+36, "DSL-N55U", 8)==0)	
#elif HW_ANNEX_B == 1
	if (strncmp(buf+36, "DSL-N55U-B", 10)==0)	
#elif
	#error "no HW defined";
#endif
	{
	...
	if (ImageErr)
	{
		printf("Image error , please turn off power and restart recovery procedure\n");
		for (i=0; i<5; i++);
		{
			//LEDON();
...
static void SolveTRX(void)
{
	...
	else
	{
		...
		//printf("\n   Copy linux image[%d byte] to Flash[0x%08X].... \n", copysize, dest);
		//printf("\n=======%x %x.... \n", copysize, ntohl(hdr->ih_size));
		//count = copysize;
		// fix trx size, copysize may include tc fw
		count = ntohl(hdr->ih_size)+64;

		printf("new size is : %x\n", count);
