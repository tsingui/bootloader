...
extern void run_ethtest(int,int);
//extern void run_usbtest(int);
...
int
do_ethtest (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
// only one port could switch to test mode
// after test completed, switch to normal first and switch another port to test mode
	extern char version_string[];
	printf ("\nethtest\n");
	if (argc == 3)
	{
		...
		run_ethtest(port, mode);
	}
	else
	{
		printf ("ex: ethtest 1 1\n");
	}
	return 0;
}
...
U_BOOT_CMD(
	ethtest,	3,		1,	do_ethtest,
 	"ethtest - set ethernet to test mode, port 1-4, mode 0/1/4 (0 = normal, 1/4 = test mode 1/4)\n",
	NULL
);

/*
int
do_usbtest (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	extern char version_string[];
	printf ("\nusbtest\n");
	if (argc == 2)
