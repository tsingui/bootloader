...
int display_line_data(int *size_p,ulong *addr_p,ulong addr_tmp,ulong *length_p)
{
	...
	do {
		...
		if(addr_tmp>=CFG_FLASH_BASE&&addr_tmp<=(CFG_FLASH_BASE+info->size))
...
#ifdef SPI_FLASH
		if((*addr_p)>=CFG_FLASH_BASE&&(*addr_p)<=(CFG_FLASH_BASE+info->size))
		{
			if(length>=1024)
			{
			...
			else
			{
				...
				rc=display_line_data(size_p, &p, addr_tmp,length_p);
