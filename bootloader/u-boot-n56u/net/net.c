...
{
//	printf("ARP_TIMEOUT: %d, ARP_TIMEOUT_COUNT: %d\n", ARP_TIMEOUT, ARP_TIMEOUT_COUNT);
...
restart:
	...
	case RARP:
		 ...
		 */
		NetOurIP = 0;
//		NetServerIP = getenv_IPaddr ("serverip");	// ASUS
...
NetReceive(volatile uchar * inpkt, int len)
{
	...
	case PROT_ARP:
		...
		case ARPOP_REQUEST:		/* reply with our IP address	*/
			...
			NetCopyIP(&NetTestIP, &arp->ar_data[ 6]);	// tmp test
			printf("test print\n");	// tmp test
			print_IPaddr(NetTestIP);
			printf("test print NetServerIP\n");	// tmp test
...
//#endif
		...
		if ((ntohs(arp->ar_op) != RARPOP_REPLY) ||
			...
			{
				printf("Net Server IP is not empty\n");	// tmp test
				print_IPaddr(NetServerIP);	// tmp test
			}

			memcpy (NetServerEther, &arp->ar_data[ 0], 6);

			printf(" get server ip:");	// tmp test
			print_IPaddr(NetServerIP);	// tmp test
